...
function formatSchoolYearLong(code: string, name?: string): string {
  const match = code.match(/SY?(\d{2})(\d{2})/i);
  if (match) {
    return `20${match[1]}-20${match[2]}`;
  }
  const nameMatch = name?.match(/(20\d{2})\s*[-–]\s*(20\d{2})/);
  if (nameMatch) {
...
export default async function PrintEvaluationPage({
  ...
  const hasSupervisorScores =
    responsibilities.some((r) => r.scoreSupervisor !== null && r.scoreSupervisor !== undefined) ||
    skills.some((s) => s.scoreSupervisor !== null && s.scoreSupervisor !== undefined);
  const isSupervisorScoringDone =
    hasSupervisorScores && totalSupervisor > 0 && evaluation.status === 'COMPLETED';
  const gradeInfo = isSupervisorScoringDone ? calculateGrade(totalSupervisor) : null;
      ...
      <div>
        ...
        <div className="avoid-break mb-3">
          ...
          <div className="text-center my-2">
            ...
            <div className="text-xs font-semibold text-slate-700">
              School Year: {formatSchoolYearLong(evaluation.schoolYear.code, evaluation.schoolYear.name)}
            </div>
          </div>
        ...
        <div className="avoid-break mb-3">
          ...
          <table className="w-full border border-slate-800 text-xs">
            ...
            <tbody>
              ...
              {/* Blank rows if fewer than 6 responsibilities to keep stable table structure */}
              {Array.from({ length: Math.max(0, 4 - responsibilities.length) }).map((_, i) => (
        ...
        <div className="avoid-break mb-3">
          ...
          <div className="bg-[#EDEDED] border-x border-b border-slate-800 p-1.5 text-[10px] text-slate-800 leading-tight">
            ...
            {gradeInfo ? (
              ...
              <span className="ml-2 font-semibold text-slate-500 italic">
                [Result: Pending Supervisor Evaluation]
              </span>
            )}
          </div>
        </div>
        ...
        <div className="avoid-break mb-3">
          ...
          <table className="w-full border border-slate-800 text-xs">
            ...
            <tbody>
              <tr>
                <td className="border border-slate-800 p-2 align-top h-20 text-[10.5px] leading-relaxed">
        ...
        <div className="avoid-break pt-2 border-t border-slate-300 text-[8.5px] text-slate-500 flex items-center justify-between font-mono">
          <div>
